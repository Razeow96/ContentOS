...
4. **Output shape is fixed by contract.** The `TrendDetected` payload must match the Linear "TrendDetected v1" contract. Fields: raw_trend_id, page, source, adapter_type, timeframe, topic, description, category, keywords, region, country, language, volume{value,unit,source}, rank, signal_type, image_url, related, external_id, url, detected_at, raw. `topic`, `page`, `source`, `raw_trend_id`, `detected_at` are required.
